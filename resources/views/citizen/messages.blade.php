...
<head>
    ...
    <title>Messagerie Citoyen - e-Déclaration TG</title>
    ...
    <style>
        * {
        ...
        /* Badge des messages non lus dans l'en-tête */
        .unread-badge {
            position: absolute;
            top: -6px;
            right: -6px;
...
<body>
    <div class="app-header">
        ...
        <div class="header-right">
            ...
            <div class="messages-icon-wrapper">
                <button class="icon-btn" onclick="location.reload()" title="Voir les messages">
            ...
            <div class="main-layout">
                ...
                <div class="citizen-sidebar">
                    <div class="sidebar-header">
                        <h2><i class="bi bi-people-fill"></i> Liste des agents</h2>
                        <div class="search-box">
                            <i class="bi bi-search"></i>
                            <input type="text" id="searchAgent" placeholder="Rechercher par nom...">
                        </div>
                    </div>
                    <div class="citizen-list" id="agentListContainer">
                ...
                <div class="chat-area">
                    <div class="chat-header">
                        <div class="chat-contact">
                            <h3 id="chatContactName">Sélectionnez un agent</h3>
                            <p id="chatContactMeta"></p>
                        </div>
                        <i class="bi bi-telephone-fill"></i>
                    </div>
                    <div class="chat-messages" id="chatMessagesContainer">
                        <div class="placeholder">Cliquez sur un agent pour voir la conversation</div>
                    </div>
    ...
    <script>
        ...
        let agentsData = @json($agents);
        let currentAgentId = null;
        let messageCount = 0; // pour détecter les nouveaux messages

        console.log('🔍 Données agents reçues :', agentsData.length, agentsData);
        ...
        function renderAgentList(data) {
            ...
            if (!data || data.length === 0) {
                html = '<div class="loading">Aucun agent trouvé</div>';
            } else {
                data.forEach(agent => {
                    let activeClass = (currentAgentId == agent.id) ? 'active' : '';
                    let statusClass = agent.status === 'online' ? 'online' : 'offline';
                    let lastMsg = agent.lastMessagePreview || 'Aucun message';
                    html += `
                        <div class="citizen-item ${activeClass}" data-id="${agent.id}">
                            <div class="avatar">${escapeHtml(agent.avatarInitials)}</div>
            ...
            $('#agentListContainer').html(html);
            $('.citizen-item').click(function() {
                let id = $(this).data('id');
                console.log('🔔 Clic sur agent ID :', id);
                selectAgent(id);
            });
        }
        ...
        function selectAgent(agentId) {
            ...
            let selected = agentsData.find(a => a.id == agentId);
            if (selected) {
                $('#chatContactName').text(selected.name);
                $('#chatContactMeta').html('<span class="status-dot online"></span> En ligne · Agent');
            } else {
                $('#chatContactName').text('Agent');
        ...
        function loadMessages(agentId) {
            $.get('{{ route("citoyen.messages.history", "") }}/' + agentId, function(messages) {
                let html = '';
        ...
        function checkNewMessages() {
            ...
            $.get('{{ route("citoyen.messages.history", "") }}/' + currentAgentId, function(messages) {
                if (messages.length > messageCount) {
                    // Nouveaux messages détectés
                    let html = $('#chatMessagesContainer').html();
                    // Recharger tous les messages pour garder la cohérence
        ...
        function sendMessage() {
            ...
            if (!message || !currentAgentId) {
                console.log('⛔ Message vide ou aucun agent sélectionné');
                return;
            }
            $.post('{{ route("citoyen.messages.envoyer") }}', {
                _token: '{{ csrf_token() }}',
                ...
                if (res.success) {
                    ...
                    $('#messageInput').val('');
                    messageCount++;
                    let agent = agentsData.find(a => a.id == currentAgentId);
                    if (agent) {
                        agent.lastMessagePreview = message;
                        renderAgentList(agentsData);
                    }
                } else {
        ...
        // Récupérer le nombre de messages non lus (badge)
        function refreshUnreadCount() {
            fetch('{{ route("citoyen.messages.unread-count") }}')
        ...
        $('#searchAgent').on('input', function() {
            ...
            let filtered = agentsData.filter(a => a.name.toLowerCase().includes(search));
